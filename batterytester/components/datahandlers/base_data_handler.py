...
def create_report_file(test_name, report_name, output_path, extension="md"):
    ...
    _filename = "{}_{}.{}".format(_base_filename, get_current_time_string(), extension)
...
class BaseDataHandler(metaclass=ABCMeta):
    ...
    def get_subscriptions(self):
        ...
        if self.subscription_filters:
            subs = (
                sub for sub in self.subscriptions if sub[0] in self.subscription_filters
            )
...
class FileBasedDataHandler(BaseDataHandler):
    def __init__(
        self, report_name, output_path, subscriptions: Optional[Subscriptions] = None
    ):
        """
